            ...
            async with self.session.get(url, params=params) as response:
                if response.status == 200:
                    ...
                    if data["retCode"] == 0:
                        ...
                        logger.warning(f"⚠️ No kline data for {symbol}: {data.get('retMsg', 'Unknown error')}")
                        return None
                else:
        ...
        try:
            ...
            async with self.session.post(url, json=params, headers=headers) as response:
                if response.status == 200:
                    data = await response.json()
                    if data["retCode"] == 0:
                        logger.info(f"✅ Orden colocada: {order_data.get('symbol')} {order_data.get('side')}")
                        return data["result"]
                    else:
                        logger.error(f"❌ Error placing order: {data.get('retMsg', 'Unknown error')}")
                        return None
                else:
