...
from config import (
    LEVERAGE, RISK_PER_TRADE, ACCOUNT_BALANCE, MAX_POSITION_SIZE,
    BASE_STOP_PERCENTAGE, MIN_STOP_DISTANCE, MAX_STOP_DISTANCE,
    RSI_OVERSOLD, RSI_OVERBOUGHT, DEFAULT_TIMEFRAMES,
    EMA_SHORT_PERIOD, EMA_LONG_PERIOD, ATR_PERIOD,
    EXTENDED_MONITORING_CONDITIONS, REACTIVATION_THRESHOLDS
)
...
class TrendAnalyzer:
    ...
    def analyze_signal(self, signal_data: Dict, symbol: str) -> Dict[str, any]:
        ...
        try:
            ...
            divergences = self.divergence_detector.analyze_divergences(symbol, technical_analysis)
            
            # 3. Análisis de confirmación de señal
            confirmation_result = self._analyze_signal_confirmation(signal_data, technical_analysis)
            ...
            logger.info(f"✅ Análisis completado para {symbol}: {recommendation.action} (x{signal_data.get('leverage', LEVERAGE)})")
            return result
    ...
    def _analyze_signal_confirmation(self, signal_data: Dict, technical_analysis: Dict) -> Dict[str, any]:
        """
        ...
        match_percentage = (trend_matches / total_timeframes * 100) if total_timeframes > 0 else 0
        ...
        logger.debug(f"📊 Confirmación: {status} ({confidence}) - Factores: {confirmation_factors}/{total_factors}")
        return result
    
    def _analyze_volume_confirmation(self, technical_analysis: Dict, signal_direction: str) -> Dict[str, any]:
        """
        ...
        try:
            ...
            if total_tfs > 0 and (trend_consistency / total_tfs) >= 0.7:
                ...
                volume_analysis['message'] = 'Tendencias consistentes sugieren volumen confirmatorio'
            
            logger.debug(f"📊 Análisis de volumen simulado: {volume_analysis['volume_confirms']}")
    ...
    def _generate_recommendation(self, signal_data: Dict, technical_analysis: Dict,
                              divergences: List, confirmation_result: Dict) -> TradingRecommendation:
        """
        ...
        strong_divergences = [d for d in divergences if d.strength == 'strong']  # ✅ CORREGIDO: usar atributo directamente
        
        if strong_divergences:
            strongest_div = strong_divergences[0]
            divergence_type = strongest_div.type  # ✅ CORREGIDO: usar atributo directamente
            if (divergence_type == 'bullish' and signal_direction == 'SHORT' or
                divergence_type == 'bearish' and signal_direction == 'LONG'):
                return TradingRecommendation(
                    action="ESPERAR",
                    confidence="ALTA",
        ...
        if confirmation_status == "NO CONFIRMADA":
            ...
            if price_diff_pct <= 2:  # Si está cerca del entry original
                stop_loss = self._calculate_stop_loss(current_price, signal_direction, technical_analysis, leverage)
                position_info = calculate_position_size(current_price, stop_loss, leverage=leverage)
            ...
            stop_loss = self._calculate_stop_loss(optimized_entry, signal_direction, technical_analysis, leverage)
            position_info = calculate_position_size(optimized_entry, stop_loss, leverage=leverage)
    ...
    def _calculate_optimized_entry(self, current_price: float, signal_entry: float, 
                                 direction: str, technical_analysis: Dict) -> float:
        """
        ...
        try:
            ...
            if price_diff_pct <= 0.5:
                logger.info(f"✅ Precio actual favorable para entry inmediato: {current_price}")
            ...
            if optimized_diff_pct > 5:  # Máximo 5% de desviación
                logger.warning(f"⚠️ Entry optimizado muy lejano ({optimized_diff_pct:.1f}%), usando original")
                return signal_entry
            
            logger.info(f"🎯 Entry optimizado: {optimal_entry} (original: {signal_entry})")
    ...
    def _find_nearby_support_resistance(self, current_price: float, direction: str, 
                                      technical_analysis: Dict) -> Dict[str, List[float]]:
        """
        ...
        try:
            ...
            logger.debug(f"📊 Niveles encontrados - Soportes: {levels['support']}, Resistencias: {levels['resistance']}")
            return levels
    ...
    def _find_optimal_long_entry(self, current_price: float, original_entry: float,
                               levels: Dict[str, List[float]]) -> float:
        """
    ...
    def _find_optimal_short_entry(self, current_price: float, original_entry: float,
                                levels: Dict[str, List[float]]) -> float:
        """
    ...
    def _calculate_stop_loss(self, entry_price: float, direction: str, 
                           technical_analysis: Dict, leverage: int = LEVERAGE) -> float:
        """
        ...
        try:
            ...
            key_levels = self._find_key_stop_levels(entry_price, direction, technical_analysis)
            
            # 2. Calcular stop basado en niveles técnicos si están disponibles
            technical_stop = self._calculate_technical_stop(entry_price, direction, key_levels)
            
            if technical_stop:
                # Validar que el stop técnico sea seguro con el apalancamiento
                stop_distance_pct = abs(entry_price - technical_stop) / entry_price * 100
                leverage_adjusted_max = MAX_STOP_DISTANCE / (leverage / 10)  # Ajustado por leverage
            ...
            return self._calculate_leverage_adjusted_stop(entry_price, direction, atr_multiplier, leverage)
            
        except Exception as e:
            logger.error(f"❌ Error calculando stop loss: {e}")
            # Fallback conservador con apalancamiento
            return self._calculate_conservative_leverage_stop(entry_price, direction, leverage)
    
    def _calculate_leverage_adjusted_stop(self, entry_price: float, direction: str, 
                                       atr_multiplier: float, leverage: int) -> float:
        """
    ...
    def _calculate_conservative_leverage_stop(self, entry_price: float, direction: str, leverage: int) -> float:
    ...
    def _find_key_stop_levels(self, entry_price: float, direction: str,
                            technical_analysis: Dict) -> Dict[str, float]:
        """
        ...
        try:
            ...
            if ema_long_levels:
                if direction == "LONG":
                    ...
                    levels['key_resistance'] = min(ema_long_levels)  # Resistencia más fuerte
    ...
    def _calculate_technical_stop(self, entry_price: float, direction: str,
                                key_levels: Dict[str, float]) -> float:
        """
        ...
        try:
            if direction == "LONG" and 'key_support' in key_levels:
                ...
                stop_buffer = resistance_level * 0.005  # 0.5% por encima de la resistencia
    ...
    def get_analysis_summary(self, analysis_result: Dict) -> Dict[str, any]:
        ...
        return {
            ...
            'predominant_trend': tech_analysis.get('consolidated', {}).get('predominant_trend', 'N/A'),
            'avg_rsi': tech_analysis.get('consolidated', {}).get('avg_rsi', 'N/A'),
            'predominant_macd': tech_analysis.get('consolidated', {}).get('predominant_macd', 'N/A'),
            'max_atr_multiplier': tech_analysis.get('consolidated', {}).get('max_atr_multiplier', 'N/A')
        }
