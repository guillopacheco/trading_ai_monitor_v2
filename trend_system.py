...
- Integración con otros módulos (si se necesita).
...
def _normalize_timeframes() -> list[str]:
    """
    Convierte DEFAULT_TIMEFRAMES de config.py (["1", "5", "15"]) al formato usado
    por get_technical_data() (["1m", "5m", "15m"]).
    """
    tfs = []
...
def _classify_confidence(match_ratio: float, smart_conf_avg: float) -> str:
    ...
    - smart_conf_avg (confianza media de las divergencias inteligentes)
    """
    # peso mayor a match_ratio, complementado por divergencias
...
def analyze_trend_core(
    ...
    - bias y confianza smart (smart_divergences)
    ...
    try:
        ...
        # ------------------------------------------------------------
        # 📈 Divergencias (desde divergence_detector -> usa smart internamente)
        # ------------------------------------------------------------
        divergences = detect_divergences(symbol, tech_multi)
        # divergences: {"RSI": str, "MACD": str, "Volumen": str}
        ...
        if smart_biases:
            # forma simple: tomar el primero más común
            # podríamos ponderar por confianza, pero mantenemos simple y estable
        ...
        if direction_hint:
            if match_ratio >= 80:
                ...
                recommendation = f"⚠️ Señal no confirmada ({match_ratio:.1f}% de coincidencia)."
        else:
            # Sin dirección propuesta, damos lectura descriptiva
...
def analyze_and_format(
    ...
    tf_lines = []
    # Ordenar por timeframe numérico si es posible
    def _tf_key(tf_name: str) -> int:
        # tf_name típico: "1m", "5m", "15m"
        try:
...
if __name__ == "__main__":
    ...
    print("🚀 Test local de trend_system.analyze_and_format()")
    r, msg = analyze_and_format("BTCUSDT", direction_hint="long")
    print(msg)
